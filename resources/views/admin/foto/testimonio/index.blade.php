...
<div class="w-full flex justify-between items-center max-w-7xl mx-auto">
  ...
  <a wire:navigate href="{{ route('admin.testimonio.create') }}">
    <button
        ...
        <table class="w-full text-sm text-left">
          <thead class="uppercase bg-light-100 dark:bg-gray-700 dark:text-gray-400">
            <tr>
              ...
              <th scope="col" class="px-4 py-3">Testimonio</th>
          ...
          <tbody>
            ...
            <tr class="border-b dark:border-gray-700 ">
              <th scope="row" class="px-4 py-3 font-medium whitespace-nowrap dark:text-white">{{ $testimonio->id
                }}</th>
              <td class="px-4 py-3">{{ $testimonio->testimonio }}</td>
              <td class="px-4 py-3">{{ $testimonio->autor }}</td>
              <td class="px-4 py-3">
              {{$testimonio->red_social}}
              </td>
              <td class="px-4 py-3 flex items-center justify-end">
                <div class="flex items-center gap-2">
                  @can('testimonio foto edit')
                    
                  <a href="{{ route('admin.testimonio.edit', $testimonio) }}" wire:navigate>
                  ...
                  @endcan
                  @can('testimonio foto delete')
                    
                  <form class="delete-form" action="{{ route('admin.testimonio.destroy', $testimonio) }}" method="POST">
                    @csrf
                    @method('DELETE')
