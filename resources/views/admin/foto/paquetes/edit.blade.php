...
<x-ui.bredcrumb :items="[
    ['url' => route('admin.paquete.index'), 'label' => 'Paquetes', 'navigate' => true],
    ['url' => '', 'label' => 'Editar Paquete']  {{-- Último elemento desactivado --}}
]" />
...
<div class="bg-light-200 dark:bg-cont-100 p-5 rounded-lg">
  <div class="">
    <div class="">
      <h3 class="text-3xl font-bold">Editar Paquete</h3>
    </div>
    <div class="mt-5">
      <form action="{{ route('admin.paquete.update', $paquete) }}" method="POST" noValidate class="space-y-3"
        >
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
          ...
          <input id="name" name="descripcion" value="{{ old('descripcion', $paquete->descripcion) }}" type="text"
        ...
        <div>
          ...
          <input id="name" name="precio" value="{{ old('precio', $paquete->precio) }}" type="text"
            placeholder="precio" class="disabled:bg-nav-900 disabled:border-nav-900 border-link-100 focus:shadow-link-200 w-full rounded-md border-2 bg-transparent p-2 outline-none focus:shadow-md placeholder:text-slate-300 dark:placeholder:text-slate-400 mt-2 @error('descripcion_corta')
          dark:border-alerts-500
          @enderror">
          @error('precio')
        ...
        <div>
          ...
          <div class="" id="editor">
             {!!old('contenido', $paquete->contenido)!!}
          </div>
          @error('contenido')
          <p class="text-sm font-semibold text-alerts-500">{{ $message }}</p>
          @enderror
