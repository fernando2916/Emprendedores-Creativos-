...
<x-ui.bredcrumb :items="[
    ['url' => route('admin.privacy.index'), 'label' => 'Aviso de privacidad', 'navigate' => true],
    ['url' => '', 'label' => 'Editar Aviso de privacidad']  {{-- Último elemento desactivado --}}
]" />
    ...
    <div class="mt-5">
      <form action="{{ route('admin.privacy.update', $privacy) }}" method="POST" noValidate class="space-y-3"
        ...
        <div>
          ...
          <input id="name" name="fecha" value="{{ old('fecha', $privacy->fecha?->format('Y-m-d')) }}" type="date"
        ...
        <div>
            ...
            <div class="" id="editor">
                {!!old('contenido', $privacy->contenido)!!}
            </div>
            @error('contenido')
            <p class="text-sm font-semibold text-alerts-500">{{ $message }}</p>
            @enderror
