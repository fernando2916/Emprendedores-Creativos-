...
<x-ui.bredcrumb :items="[
    ...
    ['url' => '', 'label' => 'Editar Terminos y condiciones']  {{-- Último elemento desactivado --}}
]" />
    ...
    <div class="mt-5">
      <form action="{{ route('admin.terminos.update', $terminos) }}" method="POST" noValidate class="space-y-3"
        ...
        <div>
          ...
          <input id="name" name="fecha" value="{{ old('fecha', $terminos->fecha?->format('Y-m-d')) }}" type="date"
        ...
        <div>
            ...
            <div class="" id="editor">
                {!!old('contenido', $terminos->contenido)!!}
            </div>
            @error('contenido')
            <p class="text-sm font-semibold text-alerts-500">{{ $message }}</p>
            @enderror
