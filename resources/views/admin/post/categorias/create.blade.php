...
<x-ui.bredcrumb :items="[
    ...
    ['url' => '', 'label' => 'Crear categoria']  {{-- Último elemento desactivado --}}
]" />

<div class="bg-light-200 dark:bg-cont-100 p-5 rounded-lg">
    <div class="">
        <div class="">
            <h3 class="text-3xl font-bold">Crear Categoria</h3>
        </div>
        <div class="">
            <form action="{{ route('admin.categories.store') }}" method="POST" noValidate>
                ...
                <div>
                    ...
                    <input id="name" name="nombre" value="{{ old('nombre') }}" type="text" placeholder="Tecnologia" class="disabled:bg-nav-900 disabled:border-nav-900 border-link-100 focus:shadow-link-200 w-full rounded-md border-2 bg-transparent p-2 outline-none focus:shadow-md placeholder:text-slate-900 dark:placeholder:text-slate-400 mt-2 @error('nombre')
