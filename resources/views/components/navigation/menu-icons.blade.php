<nav class="flex items-center gap-2">
    ...
    <button id="dropdownDelayButton" data-dropdown-toggle="dropdownDelay3" data-dropdown-delay="500"
        ...
        class="font-medium rounded-lg text-center  items-center gap-1 cursor-pointer hidden md:flex" type="button">
        <i class="fa-solid fa-user"></i>
    </button>

    <!-- Dropdown empresa -->
    <div class="z-10 hidden bg-light-300 rounded-lg shadow-sm w-64 dark:bg-nav-800" id="dropdownDelay3">
        <div class="p-1 space-y-0.5">
            <div
                ...
                <div>
                    <a wire:navigate href="{{ route('perfil.index', ['user' => auth()->user()->username]) }}"  class="flex items-center gap-x-3">
            ...
            <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg  dark:text-white hover:bg-light-200 focus:outline-hidden focus:bg-gray-100 text-slate-200 dark:hover:bg-nav-700"
                wire:navigate {{-- href="{{ route("facturacion.index") }}" --}}>
                <i class="fa-solid fa-file-export"></i>
                Facturación
            </a>
            <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg  dark:text-white hover:bg-light-200 focus:outline-hidden focus:bg-gray-100 text-slate-200 dark:hover:bg-nav-700"
                wire:navigate {{-- href="{{ route("soporte.index") }}" --}}>
            ...
            <form action=" {{ route('logout') }}" method="post">
                ...
                @csrf
                <button type="submit"
                    class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm dark:text-white hover:bg-slate-400 focus:outline-hidden focus:bg-gray-100 text-slate-200 dark:hover:bg-nav-700 w-full cursor-pointer">
