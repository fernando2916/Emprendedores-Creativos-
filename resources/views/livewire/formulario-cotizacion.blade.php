    ...
    <div class="fixed inset-0 flex items-center justify-center z-50 bg-nav-950/90">
        <div class="bg-light-300 dark:bg-cont-300 rounded p-6 w-full max-w-lg relative mx-5">
            <button wire:click="$set('modalAbierto', false)"
                class="absolute top-3 right-3 w-8 h-8 bg-btn-200 hover:bg-btn-400 dark:bg-btn-400 text-white dark:hover:bg-btn-600 duration-300 transition-colors rounded-md cursor-pointer">
                <i class="fa-solid fa-xmark text-lg px-1.5"></i>
            </button>

            <h2 class="text-xl font-semibold mb-4">Cotizar un proyecto</h2>
            ...
            <form wire:submit.prevent="guardar" class="space-y-4">
                ...
                <div>
                    <label
                        class="mb-2 text-sm font-medium text-white after:ml-0.5 after:text-red-500 after:content-['*']">Detalles
                        de tu proyecto</label>
                    <textarea wire:model.defer="detalles" placeholder="Escribe todos los detalles de tu proyecto"
                ...
                <div>
                    <label
                        ...
                        un plan</label>
                    <select wire:model.defer="plan_desing_id"
                        class="border border-link-300 text-gray-900 text-sm rounded-md focus:ring-link-500 focus:border-link-500 block w-full p-2.5 dark:bg-gray-800 dark:border-link-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-link-500 dark:focus:border-link-500 ">
                        <option value="" class="text-center">-- Selecciona --</option>
                        @foreach($planes as $plan)
                        <option value="{{ $plan->id }}">{{ $plan->titulo }}</option>
                        @endforeach
                    </select>
                    @error('plan_desing_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>
